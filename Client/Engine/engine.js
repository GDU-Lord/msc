...
const RectJS = function (fnc = () => {}, sourceHOST = '', engineSource = 'Engine/', pluginSource = 'Plugins/', eventDetector = null) {
	...
	this.Asset = function ({
		...
		return function (p) {
			...
			if(type != "Text" && typeof p.origin != "undefined") {
	...
	this.Family.prototype.forNearTo = function (_pos, fnc, dist = 100, mouse_mode = true) {
		const family = this;
		if(mouse_mode) {
			family.for(o => {
			...
			family.for(o => {
				if(Math.pow(_pos.x-o.pos.x, 2) + Math.pow(_pos.y-o.pos.y, 2) <= Math.pow(dist, 2))
					fnc(o);
			});
		}
	};
